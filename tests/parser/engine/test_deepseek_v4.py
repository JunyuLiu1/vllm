...
# content ─────────────────────────────────────────────────────────────
#
# DeepSeek V4 enables this compatibility fallback by default. It copies
# reasoning only on a confirmed stop with no reasoning end or tool boundary.
# An explicit force_nonempty_content=False disables it for either API path.


class TestForceNonemptyContentNonStreaming:
    """Non-streaming fallback requires terminal metadata and preserves reasoning."""
    ...
    def test_misroute_flushed(self, mock_tokenizer, mock_request):
        ...
        reasoning, content = parser.extract_reasoning_with_finish_reason(
            text, mock_request, finish_reason="stop"
        )

        assert reasoning == text
        assert content == text

    def test_no_flush_when_explicitly_disabled(self, mock_tokenizer, mock_request):
        parser = DeepSeekV4Parser(
            mock_tokenizer, chat_template_kwargs={"thinking": True}
        )
        mock_request.chat_template_kwargs = {"force_nonempty_content": False}
        text = "Good morning! How can I help you today?"
    ...
    def test_legacy_non_streaming_without_finish_reason_does_not_promote(
        self, mock_tokenizer, mock_request
    ):
        """Legacy callers cannot distinguish stop from truncation."""
        parser = DeepSeekV4Parser(
            mock_tokenizer, chat_template_kwargs={"thinking": True}
        )
        mock_request.chat_template_kwargs = {"force_nonempty_content": True}
...
class TestForceNonemptyContentStreaming:
    ...
    def test_no_flush_when_explicitly_disabled(self, chunk_size):
        text = "Good morning! How can I help you today?"
        tokens = _word_tokens(text)
        tokenizer = MockTokenizer(vocab=dict(_DSV4_FULL_VOCAB), tokens=tokens)
        parser = _DeepSeekV4Delegating(
        ...
        deltas = replay_streaming(
            ...
            finished_on_last=True,
            finish_reason="stop",
            chat_template_kwargs={"force_nonempty_content": False},
        )
        output = collect_output(deltas)
